# Shared makefile for building TF-M projects for TI devices. This file is included by
# device-specific Makefiles, which define the project name and device family.

# Set empty default values for variables that may not be defined by all projects
TI_DEVICE_VARIANT ?=

# For native Windows build, modify BUILD_DIR to change $(SDK_INSTALL_DIR) to 'C:' to avoid errors due to path length limitation.
BUILD_DIR := $(SDK_INSTALL_DIR)/tfm_s/build/$(if $(TI_DEVICE_VARIANT),$(TI_DEVICE_VARIANT),$(DEVICE_FAMILY))/$(PROJECT_NAME)
...
NS_BUILD_DIR := $(SDK_INSTALL_DIR)/tests/$(PROJECT_NAME)/build/$(if $(TI_DEVICE_VARIANT),$(TI_DEVICE_VARIANT),$(DEVICE_FAMILY))
...
clean:
	@ echo "$(COLOR_BLUE)Cleaning $(if $(TI_DEVICE_VARIANT),$(TI_DEVICE_VARIANT),$(DEVICE_FAMILY)) $(PROJECT_NAME) project...$(COLOR_RESET)"
...
generate: check_environment
	@ echo "$(COLOR_BLUE)Generating $(if $(TI_DEVICE_VARIANT),$(TI_DEVICE_VARIANT),$(DEVICE_FAMILY)) $(PROJECT_NAME) project...$(COLOR_RESET)"
	@ $(PYTHON) $(TFM_ROOT)/tools/tfm_gen.py \
		$(if $(TI_DEVICE_VARIANT),memory_layout_$(TI_DEVICE_VARIANT).yaml,memory_layout.yaml) $(TFM_ROOT) \
		platform/ext/target/ti/$(DEVICE_FAMILY)/partition/flash_layout.h \
		$(GENERATED_FLASH_LAYOUT_DIR)/flash_layout.h

configure: generate
	@ echo "$(COLOR_BLUE)Configuring $(if $(TI_DEVICE_VARIANT),$(TI_DEVICE_VARIANT),$(DEVICE_FAMILY)) $(PROJECT_NAME) project...$(COLOR_RESET)"
	cd $(TFM_ROOT) && $(CMAKE) -S . -B $(BUILD_DIR) \
		...
		$(if $(TI_DEVICE_VARIANT),-DTI_DEVICE_VARIANT=$(TI_DEVICE_VARIANT)) \
...
build: configure
	@ echo "$(COLOR_BLUE)Building $(if $(TI_DEVICE_VARIANT),$(TI_DEVICE_VARIANT),$(DEVICE_FAMILY)) $(PROJECT_NAME) project...$(COLOR_RESET)"
...
configure_regress_ns:
	@ echo "$(COLOR_BLUE)Configuring $(if $(TI_DEVICE_VARIANT),$(TI_DEVICE_VARIANT),$(DEVICE_FAMILY)) $(PROJECT_NAME) Non-secure project...$(COLOR_RESET)"
...
build_regress_ns: configure_regress_ns
	@ echo "$(COLOR_BLUE)Building $(if $(TI_DEVICE_VARIANT),$(TI_DEVICE_VARIANT),$(DEVICE_FAMILY)) $(PROJECT_NAME) Non-secure project...$(COLOR_RESET)"
...
.PHONY: generate configure configure_regress configure_regress_ns build build_regress build_regress_ns clean trim
