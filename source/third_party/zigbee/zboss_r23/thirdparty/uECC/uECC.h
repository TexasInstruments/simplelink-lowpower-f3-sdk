...
*/
typedef int (*uECC_RNG_Function)(zb_uint8_t *dest, unsigned size);
...
*/
int uECC_make_key(zb_uint8_t public_key[uECC_BYTES*2], zb_uint8_t private_key[uECC_BYTES]);
...
*/
zb_bool_t uECC_shared_secret(const zb_uint8_t public_key[uECC_BYTES*2],
                             const zb_uint8_t private_key[uECC_BYTES],
                             zb_uint8_t secret[uECC_BYTES]);
...
*/
int uECC_sign(const zb_uint8_t private_key[uECC_BYTES],
              const zb_uint8_t message_hash[uECC_BYTES],
              zb_uint8_t signature[uECC_BYTES*2]);
...
typedef struct uECC_HashContext {
    ...
    void (*update_hash)(struct uECC_HashContext *context,
                        const zb_uint8_t *message,
                        unsigned message_size);
    void (*finish_hash)(struct uECC_HashContext *context, zb_uint8_t *hash_result);
    unsigned block_size; /* Hash function block size in bytes, eg 64 for SHA-256. */
    unsigned result_size; /* Hash function result size in bytes, eg 32 for SHA-256. */
    zb_uint8_t *tmp; /* Must point to a buffer of at least (2 * result_size + block_size) bytes. */
} uECC_HashContext;
...
Returns 1 if the signature generated successfully, 0 if an error occurred.
*/
int uECC_sign_deterministic(const zb_uint8_t private_key[uECC_BYTES],
                            const zb_uint8_t message_hash[uECC_BYTES],
...
*/
int uECC_verify(const zb_uint8_t public_key[uECC_BYTES*2],
                const zb_uint8_t hash[uECC_BYTES],
                const zb_uint8_t signature[uECC_BYTES*2]);
...
*/
void uECC_compress(const zb_uint8_t public_key[uECC_BYTES*2], zb_uint8_t compressed[uECC_BYTES+1]);
...
*/
void uECC_decompress(const zb_uint8_t compressed[uECC_BYTES+1], zb_uint8_t public_key[uECC_BYTES*2]);
...
*/
int uECC_valid_public_key(const zb_uint8_t public_key[uECC_BYTES*2]);

#endif /* !UECC_KEY_GEN_ONLY */
...
*/
zb_bool_t uECC_compute_public_key(const zb_uint8_t private_key[uECC_BYTES],
                                  zb_uint8_t public_key[uECC_BYTES * 2]);
...
/**
 * Calculates (x mod (n-1)) + 1, according 
 * to the "DSR - Cryptographic Processing for ECDHE", J.1 step 1.b.
 * @param x     a 320-bits random value
 */
void uECC_p256_mod_n_1(const zb_uint8_t x[ZB_ECC_CURVE_P256_PRV_KEY_LEN], zb_uint8_t result[32]);
