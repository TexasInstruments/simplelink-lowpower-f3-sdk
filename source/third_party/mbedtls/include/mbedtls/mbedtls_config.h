...
/* TI-MBEDTLS: mbedtls_config.h is not a valid configuration file - modify config-hsm.h or use
 * a completely new file if configuration changes must be made. In particular, configurations
 * that introduce certain posix dependencies cannot be enabled (which they are, in this file).
 */
#error "Applications using PSA and/or mbedTLS must not use mbedTLS's default configuration file. Please define MBEDTLS_CONFIG_FILE: "<configs/config-hsm.h>" is a valid default. See the mbedTLS user's guide for details on modifying the configuration."
