...
/* TI-MBEDTLS: Casting the initial operation structs to fix compiler errors. */
#if defined(MBEDTLS_PSA_CRYPTO_CLIENT) && !defined(MBEDTLS_PSA_CRYPTO_C)
#define PSA_CIPHER_OPERATION_INIT (psa_cipher_operation_t) { 0 }
#else
#define PSA_CIPHER_OPERATION_INIT (psa_cipher_operation_t) { 0, 0, 0, 0, { 0 } }
#endif
...
#else
#define PSA_MAC_OPERATION_INIT (psa_mac_operation_t) { 0, 0, 0, { 0 } }
#endif
...
/* TI-MBEDTLS:
 * Remove PSA_KEY_ATTRIBUTES_MAYBE_SLOT_NUMBER from PSA_KEY_ATTRIBUTES_INIT
 * because we never define MBEDTLS_PSA_CRYPTO_SE_C, and the macro without
 * content causes compiler errors.
 */
// #if defined(MBEDTLS_PSA_CRYPTO_SE_C)
