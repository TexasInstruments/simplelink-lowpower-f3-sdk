...
/* TI-MBEDTLS: Modify logic for using mbedTLS vs PSA APIs for TLS key derivation - this alg
 * is not yet supported via PSA, so we must use mbedTLS APIs as the entry point. This function
 * is directly copied from the one defined below, in the #else for MBEDTLS_USE_PSA_CRYPTO.
 */
