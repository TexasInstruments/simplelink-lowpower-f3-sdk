 ...
 *
 *  Modified by Texas Instruments to include MBEDTLS_CONFIG_FILE instead of
 *  "psa/build_info.h".
 */
...
/**
 * Returns a sufficient hash suspend state buffer size for psa_hash_suspend(),
 * in bytes.
 */
#define PSA_HASH_SUSPEND_OUTPUT_SIZE(alg)                              \
    (4u /* 32-bit algorithm */ +                                       \
     ...
     (PSA_HASH_BLOCK_LENGTH(alg) - 1u) /* max unprocessed input */)

/**
 * Returns a sufficient hash suspend state buffer size for psa_hash_suspend(),
 * for any supported hash algorithms. SHA-512 is largest supported hash.
 */
#define PSA_HASH_SUSPEND_OUTPUT_MAX_SIZE                               \
    PSA_HASH_SUSPEND_OUTPUT_SIZE(PSA_ALG_SHA_512)
