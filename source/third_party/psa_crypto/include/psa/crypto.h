 ...
 *  - Replace interruptable hash signing operations with psa_hash_suspend()
 *    and psa_hash_resume().
 *  - Replace 'mbedtls_svc_key_id_t' with 'psa_key_id_t'.
 *  - Remove conditional compile for MBEDTLS_PSA_CRYPTO_STRUCT_FILE.
 */
 ...
 *   the format is defined by RFC 7748, and output is masked according to s5.
 ...
 *                              \p key. For ECDSA, the digest length must match or
 *                              exceed the curve length in security strength, per FIPS
 *                              186-4 guidelines. This means the lengths will be the same
 *                              in all cases except for SHA-512 with a 521-bit curve.
 ...
 *                              is true), that is compatible with the type of
 *                              \p key. For ECDSA, the digest length must match or
 *                              exceed the curve length in security strength. This
 *                              means the lengths will be the same in all cases except
 *                              for SHA-512 with a 521-bit curve.
 ...
 *                              the type of \p key. For ECDSA, the digest length
 *                              must match or exceed the curve length in security
 *                              strength. This means the lengths will be the same in
 *                              all cases except for SHA-512 with a 521-bit curve.
 ...
 *                          value such that #PSA_ALG_IS_SIGN_HASH(\p alg)
 *                          is true), that is compatible with
 *                          the type of \p key. For ECDSA, the digest length must
 *                          match the curve length in security strength. This means
 *                          the lengths will be the same in all cases except
 *                          for SHA-512 with a 521-bit curve.
...
#if 1 /* Texas Instruments modification */

/**
 * \brief Resume a previously suspended hash operation.
 *
 * Set up a multi-part hash operation using the hash suspend state from a
 * previously suspended hash operation.
 *
 * \param[in,out] operation    The operation object to set up. It must have
 *                             been initialized as per the documentation for
 *                             psa_hash_operation_t and not yet in use.
 * \param[in] hash_state       Buffer containing the suspended hash state
 *                             which is to be resumed.
 * \param hash_state_length    Length of \p hash_state in bytes.
 *
 * \retval #PSA_SUCCESS \emptydescription
 * \retval #PSA_ERROR_NOT_SUPPORTED \emptydescription
 * \retval #PSA_ERROR_BAD_STATE
...
/**
 * \brief Suspend an active hash operation.
 *
 * Halt the hash operation and extract the intermediate state of the hash
 * computation.
 *
 * \param[in] operation            Active hash operation.
 * \param[out] hash_state          Buffer where the hash suspend state is to be
 *                                 written.
 * \param hash_state_size          Size of the hash_state buffer in bytes. This
 *                                 must be appropriate for the selected algorithm.
 * \param[out] hash_state_length   On success, the number of bytes that make up
 *                                 the hash suspend state.
 *
 * \retval #PSA_SUCCESS
 *         Success.
 *         The first (*hash_state_length) bytes of \p hash_state contain the
 *         intermediate hash state.
 * \retval #PSA_ERROR_NOT_SUPPORTED \emptydescription
 * \retval #PSA_ERROR_BUFFER_TOO_SMALL
 *         The size of the \p hash_state buffer is too small. You can determine
 *         a sufficient buffer size by calling
 *         #PSA_HASH_SUSPEND_OUTPUT_SIZE(\c alg)
 *         where \c alg is the algorithm that was used to set up the operation.
 * \retval #PSA_ERROR_BAD_STATE
