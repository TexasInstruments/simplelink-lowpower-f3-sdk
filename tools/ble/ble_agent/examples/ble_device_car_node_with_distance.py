    ...
    if car_node.initialize():
        ...
        while continue_scan:
            ...
            if car_node.all_event_list.is_event_in_list(ConnectionEventType.NWP_LINK_ESTABLISHED_EVENT):
                ...
                car_node.central.stop_scan()
                continue_scan = 0
                connection_data = car_node.all_event_list.get_event_from_list(ConnectionEventType.NWP_LINK_ESTABLISHED_EVENT)
        ...
        if connection_data:
            ...
            if not pairing_data:
                print("Didn't get NWP_PAIRING_STATE_COMPLETE, wait for NWP_PAIRING_STATE_ENCRYPTED in case we already bonded")
...
def start_cs(car_node):
    ...
    car_node.cs.set_default_settings(conn_handle=0, role_enable=3, sync_anthenna_selection=1, max_tx_power=10)
    time.sleep(1)
    ...
    for i in range(100):
        ...
        time.sleep(0.1)
        cs_proc_enable(car_node, cs_set_procedure_params["max_procedure_count"], wait_for_remote=True, wait_for_distance=True, wait_for_raw_events=False)
    ...
    # cs_proc_enable(car_node, cs_set_procedure_params_repeat["max_procedure_count"], wait_for_remote=True, wait_for_distance=False)
...
def wait_results_subevents(car_node, wait_for_remote=True):
    ...
    while local_proc_done:
        if local_results:
            ...
            if local_results:
                print_obj_data("Subevent Results Continue - local device", local_results)
                local_proc_done = local_results['procedure_done_status']
            else:
    ...
    if wait_for_remote:
        ...
        while remote_proc_done:
            if remote_results:
                ...
                if remote_results:
                    print_obj_data("Subevent Results Continue - remote device", remote_results)
                    remote_proc_done = remote_results['procedure_done_status']
                else:
                    print("No CS_SUBEVENT_RESULTS_CONTINUE arrived")
                    remote_proc_done = False

def cs_proc_enable(car_node, repeat=0, wait_for_remote=True, wait_for_distance=True, wait_for_raw_events=False):
    """
    ...
    for i in range(repeat):
        if wait_for_raw_events:
            ...
            # Expect only distance results event
            wait_distance_event(car_node)
        else:
            break
